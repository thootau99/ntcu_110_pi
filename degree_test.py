...
def imageDegreeCheck(image, it):
    ...
    low_yellow = np.array([20,100,100]) # 20, 100 ,100
    high_yellow = np.array([30,255,255]) # 30, 255 ,255
    mask = cv2.inRange(hsv,low_yellow,high_yellow)

    # r = cv2.resize(mask, (0,0), fx=0.1, fy=0.1)
    ...
    try:
        ...
        # l = cv2.resize(edges, (0,0), fx=0.1, fy=0.1)
        # cv2.startWindowThread()
        # cv2.namedWindow("ppp")
        # cv2.imshow("ppp", l)
        # cv2.waitKey(1)
